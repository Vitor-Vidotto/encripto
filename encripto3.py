...
def generate_password_hmac(site, secret_key, checkbox_selection, use_mac):
    mac_address = get_mac_address() if use_mac else ""
    combined = f"{site}{checkbox_selection}{mac_address}".encode()
...
def show_checkbox_screen():
    ...
    return ''.join(checkbox_selection)  # Retorna os valores selecionados
...
def generate_password_gui():
    ...
    while True:
        ...
        use_mac_var = tk.BooleanVar()
        use_mac_checkbox = tk.Checkbutton(site_window, text="Usar Endereço MAC", variable=use_mac_var, font=("Arial", 12))
        use_mac_checkbox.pack(pady=10)

        def on_confirm_site():
            site = site_entry.get()
            site_window.destroy()  # Fecha a janela de digitação do site
            generate_password(site, use_checkboxes_var.get(), use_mac_var.get())
...
def generate_password(site, use_checkboxes, use_mac):
    ...
    if secret_key:
        ...
        password = generate_password_hmac(site, secret_key, checkbox_selection, use_mac)
